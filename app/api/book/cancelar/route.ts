...
export async function POST(req: NextRequest) {
  ...
  // Fetch cita info before cancelling (for notifications)
  const { data: citaData } = await supabase.rpc('get_cita_por_token', { p_token: token })
  const cita = citaData as {
    ok: boolean; cita_id?: string; inicio?: string; fin?: string
  ...
  const { data, error } = await supabase.rpc('cancelar_cita_por_token', { p_cancel_token: token })
