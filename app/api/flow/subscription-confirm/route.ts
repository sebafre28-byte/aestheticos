...
import { createSubscription } from '@/lib/subscriptions/flow'
...
export async function GET(request: NextRequest) {
  ...
  if (!clinica_id || !plan) {
    return NextResponse.redirect(new URL('/configuracion?tab=plan&error=parametros', request.url))
  }
  ...
  try {
    ...
    const { subscriptionId } = await createSubscription(sub.flow_customer_id, plan, anual)
    ...
    return NextResponse.redirect(new URL('/configuracion?tab=plan&checkout=success', request.url))
  } catch (err) {
    console.error('[flow/subscription-confirm]', err)
    return NextResponse.redirect(new URL('/configuracion?tab=plan&error=suscripcion', request.url))
  }
}
