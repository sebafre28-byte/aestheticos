...
import {
  ...
  AlertCircle, Eye, EyeOff, X, ChevronDown, Check, Camera,
} from 'lucide-react'
...
function Avatar({ url, initials, size = 'lg', onClick, uploading }: {
  url: string
  initials: string
  size?: 'sm' | 'lg'
  onClick?: () => void
  ...
  const dim = size === 'lg' ? 'w-20 h-20 text-[22px]' : 'w-10 h-10 text-[14px]'
  return (
    <div className={`relative ${dim} rounded-full shrink-0`}>
      <div className={`${dim} rounded-full flex items-center justify-center text-white font-bold overflow-hidden`}
        style={{ background: url ? undefined : 'linear-gradient(135deg, #2563EB 0%, #14B8A6 100%)' }}>
        {url
      ...
      {onClick && (
        <button
          ...
          onClick={onClick}
          disabled={uploading}
          className="absolute bottom-0 right-0 w-6 h-6 rounded-full bg-white border border-gray-200 shadow flex items-center justify-center hover:bg-gray-50 transition-colors disabled:opacity-60"
        >
          {uploading ? <Loader2 className="size-3 animate-spin text-gray-400" /> : <Camera className="size-3 text-gray-500" />}
        </button>
      )}
    </div>
  )
...
function SeccionPerfil() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      setEmail(user.email ?? '')
      setTelefono((meta.telefono as string) ?? '')
      setFotoUrl((meta.avatar_url as string) ?? '')
  ...
  async function guardar(e: React.FormEvent) {
    ...
    setFeedback(null)
    const supabase = createClient()
    await supabase.auth.updateUser({ data: { nombre: nombre.trim(), telefono: telefono.trim() } })
    if (rol === 'profesional' && profesionalId) {
  ...
  return (
    <form onSubmit={guardar} className="space-y-5">
      ...
      <div className="flex items-center gap-5">
        <input ref={fotoRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={handleFotoChange} />
        <Avatar url={fotoUrl} initials={initials} size="lg" onClick={() => fotoRef.current?.click()} uploading={subiendoFoto} />
        <div>
          <p className="text-[13px] font-medium text-gray-700">{nombre || 'Tu nombre'}</p>
          <p className="text-[12px] text-gray-400 mt-0.5">{email}</p>
          <p className="text-[11px] text-gray-400 mt-2">PNG, JPG o WebP · Máx 5 MB</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* Nombre */}
        <div>
          <Label className="mb-1.5 block text-[12px] font-medium text-gray-700">Nombre completo <span className="text-red-400">*</span></Label>
        ...
        <div>
          ...
          <Input value={telefono} onChange={e => setTelefono(e.target.value)} placeholder="+56 9 XXXX XXXX" inputMode="tel" className="h-9 text-[13px]" />
        </div>

        {/* Email readonly */}
        <div className="sm:col-span-2">
          <Label className="mb-1.5 block text-[12px] font-medium text-gray-700">Correo electrónico</Label>
          <Input value={email} readOnly className="h-9 text-[13px] bg-gray-50 text-gray-500 cursor-not-allowed" />
          <p className="text-[11px] text-gray-400 mt-1">El correo no se puede cambiar desde aquí.</p>
        </div>
      </div>
...
function SeccionGoogleCalendar() {
  ...
  async function cambiarCalendario(id: string) {
    ...
    if (res.ok) setFeedback({ tipo: 'ok', msg: 'Calendario actualizado.' })
    else setFeedback({ tipo: 'error', msg: 'No se pudo actualizar el calendario.' })
  }

  void syncMode; void setSyncMode; void guardandoMode; void setGuardandoMode
  ...
  return (
    <div>
      <Feedback f={feedback} />
      <div className={`rounded-xl border p-4 mb-5 ${conectado ? 'bg-emerald-50/50 border-emerald-100' : 'bg-gray-50 border-gray-100'}`}>
        <div className="flex items-center gap-3">
          ...
          <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ${conectado ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
      ...
      {conectado && tokenExpirado && (
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3.5 flex items-start gap-3">
          ...
          <a href="/api/auth/google" className="shrink-0 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-[12px] font-semibold px-3 py-1.5 transition-colors">Reconectar</a>
        </div>
      )}
      ...
      {conectado ? (
        <>
          ...
          <div ref={dropdownRef} className="relative mb-5">
            <button type="button" onClick={() => setDropdownAbierto(v => !v)}
              ...
              <div className="flex items-center gap-2 min-w-0">
                {calSeleccionado?.backgroundColor && <span className="size-2.5 rounded-full shrink-0" style={{ backgroundColor: calSeleccionado.backgroundColor }} />}
                <span className="truncate">{nombreCal}</span>
                {(calSeleccionado?.primary || calendarId === 'primary') && <span className="text-[11px] text-gray-400 shrink-0">· principal</span>}
              </div>
              {cargandoCals ? <Loader2 className="size-3.5 animate-spin text-gray-400 shrink-0" /> : <ChevronDown className="size-3.5 text-gray-400 shrink-0" />}
            </button>
            {dropdownAbierto && calendarios.length > 0 && (
              <div className="absolute z-20 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg overflow-hidden">
                {calendarios.map(cal => (
                  <button key={cal.id} type="button" onClick={() => cambiarCalendario(cal.id)}
...
function UserHeader() {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4 shrink-0">
      <Avatar url={fotoUrl} initials={initials} size="sm" />
...
function MiCuentaInner() {
  ...
  const tabTitles: Record<Tab, { title: string; subtitle: string }> = {
    perfil:         { title: 'Mi perfil', subtitle: 'Foto, nombre, teléfono y datos personales' },
