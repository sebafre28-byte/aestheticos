-- Serializa reservas simultáneas del mismo profesional con un advisory lock
-- transaccional. Evita la race condition donde dos requests pasan el chequeo
-- de disponibilidad al mismo tiempo y ambas insertan citas solapadas.
-- También agrega SET search_path = public (hardening).
...
DECLARE
  ...
  v_cancel_token uuid;
BEGIN
  -- Lock por profesional: la segunda reserva simultánea espera a que la
  -- primera termine su transacción, y entonces ve la cita ya creada.
  PERFORM pg_advisory_xact_lock(hashtext(p_profesional_id::text));
