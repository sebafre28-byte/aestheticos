...
function shell(heroGradient: string, heroIcon: string, body: string, clinicaLogo?: string | null): string {
...
export function buildCumpleanosEmail(d: {
  ...
  `
  return { subject, html: shell('linear-gradient(135deg,#EC4899 0%,#F97316 100%)', '🎂', body, d.clinica_logo_url) }
}
...
export function buildReactivacionEmail(d: {
  ...
  `
  return { subject, html: shell('linear-gradient(135deg,#2563EB 0%,#7C3AED 100%)', '💙', body, d.clinica_logo_url) }
}
