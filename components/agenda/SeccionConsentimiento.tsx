...
import { FileText, Send, CheckCircle2, Clock, AlertCircle, Loader2, ExternalLink } from 'lucide-react'
...
export function SeccionConsentimiento({ cita }: { cita: CitaConRelaciones }) {
  ...
  const hasFirmado = solicitudes.some(s => s.estado === 'firmado')
      ...
      {loading ? (
        ...
        <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg border border-green-100">
          ...
          <div className="min-w-0">
            ...
            {solicitudes.find(s => s.estado === 'firmado')?.firmado_at && (
              <p className="text-[11px] text-green-600 mt-0.5">
                {format(new Date(solicitudes.find(s => s.estado === 'firmado')!.firmado_at!), "d MMM 'a las' HH:mm", { locale: es })}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg border border-amber-100">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-amber-600 mt-0.5">
              Enviado a {latest.email_destino}
            </p>
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="flex-1 text-xs bg-blue-600 hover:bg-blue-700 gap-1"
            >
              {sending ? (
